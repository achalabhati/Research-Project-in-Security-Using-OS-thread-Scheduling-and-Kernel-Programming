...
void *parentThreads(void *data)
{
   ...
   if(tid==0){
//	printf("Address &ec_GFp_simple_add %p\n",&ec_GFp_simple_add);
	//printf("Address &ec_GFp_simple_dbl %p\n",&ec_GFp_simple_dbl);	
	}
	...
	while(1)
	{
		...
		if(counter==5 && tid ==0){ // notify V to start AES. We delayed it as to start AES at stable condition 
			*start_flag=1;
