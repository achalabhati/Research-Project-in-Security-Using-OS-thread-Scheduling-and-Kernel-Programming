...
ofile.write("p_pid	  	p_name		p_vrt	     p_sum_exe_rt  	    n_pid   	  n_name       n_vrt  	  n_sum_exe_rt|	prev_tslice next_tslice  CPU\n")
 

# All_context_switch_statistics.txt
ofile2=(open("All_context_switch_statistics.txt", 'w'))
#ofile.write("p_pid\tp_vrt\tp_sum_exe_rt\tn_pid\tn_vrt\tn_sum_exe_rt\t|\tDiff_vrtime sum_rt_next\n")
ofile2.write("p_pid     p_name		p_vrt	      p_sum_exe_rt 		  n_pid     n_name  	 n_vrt  	 n_sum_exe_rt|	prev_tslice next_tslice  CPU\n")
...
with open ("context_switch.txt") as ifile:
	...
	vrun_time2 = {}
	for line in lines:
		if len(line.split(',')) is not 8:continue
		ar = line.split(',')
		...
		if (ar[0].isdigit()):			
			...
			if last_vrun is 0 and cflag1 is 0:	
				...
				cpu_time_spent= int(ar[3]) - last_vrun
			vrun_time[int(ar[0])]=int(ar[3])

			if int(ar[4]) in vrun_time2 : 
				last_vrun2 = vrun_time2[int(ar[4])]
				cflag2=1
			...
			if last_vrun2 is 0 and cflag2 is 0:	
				...
				cpu_time_spent2= int(ar[7]) - last_vrun2
			vrun_time2[int(ar[4])]=int(ar[7])
 
			res='%6s ,%10s, %15s, %15s, %6s ,%10s, %15s,%15s, | , %10s ,%10s \n' % (ar[0],ar[1], ar[2],ar[3],ar[4],ar[5],ar[6],ar[7].strip(), str(cpu_time_spent), str(cpu_time_spent2))
			ofile2.write(res)
