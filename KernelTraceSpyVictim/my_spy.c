...
#define MAX_COUNT 25000/*MAX_COUNT should be multiple of NUM_THREADS to take threads out of the loop*/
...
void *threadA(void *data_)  // Thread function
{
    ...
    while(1)  // Long loop where spies have to run till the required number of decryption are obtained.
    {
       ...
       printf("\n my thread enter^^^^^^^^^^^^ %d access count %d counter %d",tid,*access_count,counter);
        if (ret)
        {
            printf("Error in Sem Post\n");
        }
